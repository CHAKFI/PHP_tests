...
function IdentificationDesDroits($groupes_visibles_user,$groupes_interdits_user,$is_admin,$id_fonctions,$refresh){
    ...
    if ( $is_admin == 1 ){
        ...
        foreach($fonctions_associees as $fonc_id){
            if ( !empty($fonc_id) ){
            	//Get allowed and forbiden folders for each Role
            	$data = $db->query_first("SELECT groupes_visibles,groupes_interdits FROM ".$pre."functions WHERE id=".$fonc_id);

            	//Add all allowed folders
            	$list_allowed_folders = array_merge($list_allowed_folders, explode(';',TrimElement($data['groupes_visibles'], ";")));
            	$list_forbiden_folders = array_merge($list_forbiden_folders, explode(';',TrimElement($data['groupes_interdits'], ";")));
		...
		$list_forbiden_folders = array_unique($list_forbiden_folders);

    	//Exclude from allowed folders all the forbiden folders coming from Roles definition
    	foreach($list_allowed_folders as $id){
    		if (!in_array($id,$list_forbiden_folders)){
    			array_push($allowed_folders_tmp,$id);
    		}
    	}

   		//Add user allowed folders
    	$allowed_folders_tmp =array_unique(array_merge($allowed_folders_tmp,explode(';',TrimElement($groupes_visibles_user,";"))));
    	...
    	$new_liste_gp_visibles = $list_allowed_folders;
        //Clean array
        $_SESSION['groupes_visibles'] = array_unique($new_liste_gp_visibles);
        foreach($_SESSION['groupes_visibles'] as $key => $value) {
          if($value == "") {
            unset($_SESSION['groupes_visibles'][$key]);
          }
        }
        ...
        if ( isset($_SESSION['settings']['enable_pf_feature']) && $_SESSION['settings']['enable_pf_feature'] == 1 ) {
            ...
            if ( !empty($pf[0]) ){
                if ( !in_array($pf[0],$_SESSION['groupes_visibles']) ){
                    ...
                    array_push($_SESSION['personal_visible_groups'],$pf[0]);
                    //get all descendants
                    $ids = $tree->getDescendants($pf[0]);
                    foreach($ids as $id){
                        array_push($_SESSION['groupes_visibles'], $id->id);
