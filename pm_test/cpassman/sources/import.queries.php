...
## Description : File contains queries for ajax dedicated for IMPORT functionality
## 
## DON'T CHANGE !!!
## 
####################################################################################################
...
switch($_POST['type'])
{
    ...
    case "import_file_format_keepass":  
        ...
        $name = $level_in_progress = $previous_level = $full_path = $history_level = $path = $display = "";
        ...
        */
        function Recursive_Keepass_XML($xml_root, $xml_level = 0) {
            global $meta, $root, $group, $name, $entry, $level_min, $key, $title, $notes, $pw, $username, $url, $new_item, $temp_array, $history, $level_in_progress, $history_level, $nb_items, $path, $previous_level, $generator_found, $cacheFile, $cacheFileF, $num_groups, $num_items, $folders_separator, $items_separator, $line_end_separator;
            // For each node, get the name and SimpleXML balise
            foreach($xml_root as $nom=>$elem) {                    
                if(trim($elem) == "") {
                    
                    //check if file is generated by keepass
        ...
        ##################
        if ( $num_items>0 || $num_groups>0 ){$items_array = array();
