...
if ( isset($_POST['type']) ){
    switch($_POST['type'])
    {
        ...
        case "new_item":
            ...
            if( $create_new_item == true) {
                ...
                $pw_received = password_replacement($_POST['pw']);

                $resticted_to = $_POST['restricted_to'];
                //encrypt PW
                if ( !empty($_POST['salt_key']) && isset($_POST['salt_key']) ){
                    $pw = encrypt($pw_received,mysql_real_escape_string(stripslashes($_POST['salt_key'])));
    ...
    ### update an ITEM
        case "update_item":
            ...
            //Manage specific characters (&, +)
            $pw_recu = password_replacement($_POST['pw']);
            ...
            if ( $data_count[0] > 0 ){
                ...
                foreach( $rows as $reccord ) {
                    ...
                    if ( empty($id_managed) || $id_managed != $reccord['id'] ){
                        ...
                        array_push($items_id_list,array($reccord['id'],decrypt($reccord['pw']),$reccord['login'],$display_item));
